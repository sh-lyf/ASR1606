...
int32_t core_auth_mqtt_password(char **dest, char *product_key, char *device_name, char *device_secret)
{
    ...
    res = core_sprintf(&plain_text, "clientId%s.%sdeviceName%sproductKey%stimestamp%s", src, sizeof(src)/sizeof(char *));
    if (res < STATE_SUCCESS) {
        return res;
    }
...
int32_t core_auth_mqtt_clientid(char **dest, char *product_key, char *device_name, char *secure_mode, char *extend_clientid)
{
    ...
    return core_sprintf(dest, "%s.%s|timestamp=%s,_ss=1,_v=%s,securemode=%s,signmethod=hmacsha256,ext=3,%s|", src, sizeof(src)/sizeof(char *));    /* ext bitmap: bit0-rrpc, bit1-ext_notify */
}
#if 0
